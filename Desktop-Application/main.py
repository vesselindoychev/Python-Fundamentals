...
def render_main_view():
    clear_view()
    Button(tk, text="List all tasks", bg="red", fg="black", command=render_list_tasks_view).grid(column=0, row=0,
                                                                                                 padx=20, pady=20)
    Button(tk, text="Create task", bg="green", fg="black", command=render_create_view).grid(column=1, row=0, padx=20,
                                                                                            pady=20)
...
def render_list_tasks_view():
    ...
    dropdown = Combobox(tk, textvariable=selected_task, width=70)
...
def render_create_view():
    ...
    Label(tk, text="Check if the task is completed?").grid(column=0, row=3, padx=20, pady=20)
    # TODO make check boxes
...
if __name__ == "__main__":
    tk = Tk()
    tk.geometry("700x500")
    tk.title("tkinter    Doychev's First Desktop App Project")
    render_main_view()
    tk.mainloop()
